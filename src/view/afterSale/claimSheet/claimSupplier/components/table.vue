<template>
  <section class="oper-top">
    ...
    >
      ...
      <vxe-table-column field="treatedThisQty" title="本次处理数量" width="80">
        <template v-slot="{row}">
          <vxe-input type="integer" v-model="row.treatedThisQty" :min="1" :max="row.untreatedQty"/>
        </template>
      </vxe-table-column>
