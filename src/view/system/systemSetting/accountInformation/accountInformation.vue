<template>
  <div class="bigbox2">
    <div class="header">
      <Button type="warning" class="mr15" @click="recharge">
        <span class="center">
          <Icon custom="iconfont iconziyuan12 icons" />华币充值
        </span>
      </Button>
      <Button class="mr10" @click="record">
        <span class="center">
          <Icon custom="iconfont iconziyuan10 icons" />充值消费记录
        </span>
      </Button>
    </div>
    <div class="boxnav">
      <div class="navone2">
        ...
        <div class="navationTwo">
          ...
          <span>
            {{ maturity }} 剩余
            <span style="color:#40a6ff;font-weight: bold;padding: 0 5px">{{ remainingDays }}</span>天
          </span>
        </div>
        <div class="navationTwo">
          <label>华币数量:</label>
          <span>
            剩余
            <span style="color: #40a6ff;font-weight: bold;padding: 0 5px">{{ amount }}</span>个
          </span>
        </div>
        <div class="navationTwo">
          <label>负责人:</label>
            ...
            <ul class="List">
              <li class="List_item" v-for="(item,index) in UpgradeModule" :key="index">
                <Poptip trigger="hover" :content="'有效期至:'+ item.expiryDate" v-if="item.flag == 1">
                  <Button
                    shape="circle"
                    :type="[item.flag === 1? 'warning':'Default']"
                    class="colorWhite"
                    @click="ProductsBuy(item)"
                  >{{ item.name }}</Button>
                </Poptip>
                <Button
                  v-else
