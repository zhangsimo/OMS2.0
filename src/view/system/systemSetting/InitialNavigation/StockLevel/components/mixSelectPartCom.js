...
import {stockLevelPartSave} from '../../../../../../api/system/systemSetting/Initialization'
  ...
  methods:{
    ...
    throwData(){
      if(this.selectTableItem.length>0){
        ...
        let params = {}
        params.levelId = this.infoData
        let data = this.selectTableItem
        stockLevelPartSave({params:params,data:data}).then(res => {
          if(res.code === 0){
            this.$Message.warning('选择成功！');
            this.searchPartLayer = false;
            this.$parent.rightgetList()
          }
        })
      }else{
