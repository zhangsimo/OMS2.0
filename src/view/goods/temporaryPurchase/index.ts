...
export default class TemporaryPurchase extends Vue {
  ...
  private formdata(refname: string) {
    ...
    ref.validate((valid: any) => {
      if (valid) {
        ...
        data.planArriveDate = data.planArriveDate
          ? tools.transTime(data.planArriveDate)
          : "";
        data.directCompanyId = data.directCompanyId
          ? data.directCompanyId
          : "0";
    ...
    if (!obj.directCompanyId) {
      obj.directCompanyId = this.formPlanmain.directCompanyId
        ? this.formPlanmain.directCompanyId
        : "0";
    }
    return obj;
  }
  ...
  private delPart() {
    ...
    this.$Modal.confirm({
      title: "是否要删除配件",
      onOk: async () => {
        let arr = [...this.deletePartArr, ...this.tmpDeletePartArr].map(
          el => el.uuid
        );
        let res: any;
  ...
  private setFormPlanmain(v: any) {
    if (v) {
      ...
      this.formPlanmain.orderDate =
        v.orderDate instanceof Date ? v.orderDate : new Date(v.orderDate);
      this.formPlanmain.planArriveDate = v.planArriveDate
        ? new Date(v.planArriveDate)
        : "";
      ...
      if (
        ["草稿", "已作废"].includes(v.billStatusId.name) ||
        !v.processInstanceId
      ) {
        this.hideSp = true;
      } else {
  ...
  private mounted() {
    ...
    this.showSelf = Reflect.has(self, "temporaryPurchase")
      ? self.temporaryPurchase
      : true;
