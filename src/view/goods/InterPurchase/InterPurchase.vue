<template>
  <div class="content-oper content-oper-flex loadingClass">
    <section class="oper-box">
      <div class="oper-top flex">
        <div class="wlf wlf-center">
          ...
          <div class="db">
            <Button @click="print" class="mr10" :disabled="selectTableRow === null" v-has="'print'">
          ...
          <div class="db">
            <Button @click="showFee" class="mr10" v-has="'reg'">
              <i class="iconfont mr5 iconshenheicon"></i> 费用登记
            </Button>
          </div>
          <div class="db">
            <div class="mt5">
        ...
        <div class="con-split" ref="paneLeft">
          <Split v-model="split1" min="200" max="500" @on-moving="getDomHeight">
            ...
            <div slot="right" class="con-split-pane-right pl5 goods-list-form">
              <div class="pane-made-hd">采购订单信息</div>
              <div class="clearfix purchase" ref="planForm">
                ...
                >
                  ...
                  <FormItem class="form-Item" label="结算方式：" prop="settleTypeId">
                    <Select class="w160" v-model="formPlanmain.settleTypeId" :disabled="isInput">
                      <Option
                  ...
                  <FormItem class="form-Item" label="入库仓：" prop="storeId">
                    <Select class="w160" v-model="formPlanmain.storeId" :disabled="isInput">
                      <Option
              ...
              >
                ...
                >
                  ...
                  <template v-slot="{ row }">{{ row.rmbPrice | priceFilters }}</template>
                </vxe-table-column>
                <vxe-table-column show-overflow="tooltip" field="tariffAmt" title="关税费" width="100"></vxe-table-column>
                <vxe-table-column
                  show-overflow="tooltip"
                  field="transportAmt"
                  title="运杂费"
                  width="100"
                ></vxe-table-column>
                <vxe-table-column show-overflow="tooltip" field="vatAmt" title="增值税" width="100"></vxe-table-column>
                <vxe-table-column
                  show-overflow="tooltip"
                  field="otherAmt"
                  title="其他费用"
                ...
                >
                  ...
                  <template v-slot="{ row }">{{ row.otherAmt | priceFilters }}</template>
                </vxe-table-column>
                <vxe-table-column
                  show-overflow="tooltip"
                  field="orderPrice"
                ...
                >
                  <template
                    v-slot:default="{ row }"
                  >{{ (row.rmbPrice + row.tariffAmt + row.transportAmt + row.vatAmt + row.otherAmt) | priceFilters }}</template>
                </vxe-table-column>
                <vxe-table-column show-overflow="tooltip" title="采购金额" field="orderAmt" width="120">
                  <template
                    v-slot:default="{ row }"
                  >{{ ((row.rmbPrice + row.tariffAmt + row.transportAmt + row.vatAmt + row.otherAmt) * row.orderQty) | priceFilters }}</template>
                </vxe-table-column>
                <vxe-table-column
                  show-overflow="tooltip"
                  field="remark"
    ...
    <apportionment-expenses
      ...
      :parentFeeform="selectTableRow"
      :currencies="currencyMap"
      @currencyForm="getFeeForm"
    ></apportionment-expenses>
  </div>
</template>
