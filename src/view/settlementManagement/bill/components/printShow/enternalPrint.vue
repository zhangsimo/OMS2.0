            ...
            <tr>
              <th colspan="12" style="line-height: 18px;font-weight: 400;">
                <div style="display: flex; flex-flow: row nowrap">
                  ...
                  <div style="flex: 1;padding: 4px;">
                    <div style="font-size: 12px;">采购入库单号: {{ onelist.serviceId }}</div>
                    <div style="font-size: 12px;">采购订单单号: {{ onelist.code }}</div>
                  </div>
                </div>
              </th>
            </tr>
...
<script>
  ...
  import {enternalPrintApi} from "@/api/bill/saleOrder"; //采购入库 销售出库
    ...
    methods: {
      ...
      async openModal(order) {
        if (order.hasOwnProperty("id")) {
          ...
          let res = await enternalPrintApi(data);
          if (res.code == 0) {
            this.printShow = true;
            this.onelist = res.data;
            res.data.pchsEnterDetailVOS.map((item, index) => {
              item.num = index + 1;
            });
