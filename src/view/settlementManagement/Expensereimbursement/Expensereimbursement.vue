<template>
  <div class="content-oper content-oper-flex loadingClass">
    <section class="oper-box">
      ...
      <div class="mt10 mb10">
        <Button v-has="'claim'" class="ml10" @click="claimCollect"
          >报销认领</Button
        >
          ...
          >
            ...
            <vxe-table-column title="金额信息">
              ...
              <vxe-table-column
                ...
                width="120"
              ></vxe-table-column>
              <vxe-table-column title="因公借支核销单号" width="150">
                <template v-slot="{ row }">
                  <ul class="list">
                    <li
            ...
            <vxe-table-column title="其他信息">
              <vxe-table-column field="receiver" title="付款人" width="60">
                <template v-slot="{ row }">
                  <ul class="list">
                    <li
...
import {
  expensereimbursementExport /**导出*/,
} from "@/api/settlementManagement/Import";
// otherReceivables
  ...
  methods: {
    ...
    openRegEnter() {
      if (this.currRow && this.currRow.hasOwnProperty("id")) {
        ...
        this.$refs.registrationEntry.accountData.push(objItem);
    ...
    exportTable() {
      if (this.tableData.length < 1) {
        ...
        if (this.BranchstoreId == "") {
          return this.$Message.error("请选择分店");
        }
        let obj = {
    ...
    getQuery() {
      ...
      restful
        .findByDynamicQuery(params, data)
        .then((res) => {
          if (res.code == 0) {
