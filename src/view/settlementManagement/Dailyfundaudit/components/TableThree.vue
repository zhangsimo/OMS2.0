<template>
  <div>
    ...
    >
      ...
      <vxe-table-column title="往来单位" min-width="120" fixed="left">
        <template v-slot="{row}">
          <ul class="list">
      ...
      <vxe-table-column field="remarks" show-overflow='title' title="备注" min-width="60"></vxe-table-column>
    </vxe-table>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      async getList(params){
        let res=await api.table3(params)
        if(res.code===0){
          this.$parent.$parent.$parent.tableData3=res.data.content || [];
