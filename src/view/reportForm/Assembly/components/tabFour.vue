...
<!--    class="con-box"-->
    ...
    >
      ...
      <vxe-table-column  show-overflow="tooltip" field="group2" title="半成品入库配件信息">
        <vxe-table-column  show-overflow="tooltip"
          field="partCode"
          title="配件编码"
          width="300"
...
export default {
  ...
  methods: {
    ...
    async getList(data = {}) {
      ...
      let res = await api.getPartSplitEnter(data,this.page);
      if (res.code == 0) {
    ...
    async exportFun(){
      ...
      return new Promise(async (resolve, reject) => {
        let res = await api.getPartSplitEnter(this.searchData,pageObj);
        if (res.code == 0) {
