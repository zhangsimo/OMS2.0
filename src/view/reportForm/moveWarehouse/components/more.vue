...
<!--          @on-change="select1"-->
      ...
      <FormItem label="移入仓库: " v-if="type != 4">
        <Select v-model="warehouseId2" class="w300 ml5" label-in-value filterable clearable>
          <Option v-for="(item) in warehouse2" :value="item.id" :key="item.id">{{ item.name }}</Option>
        </Select>
      </FormItem>
      <FormItem label="业务员: ">
...
export default class MoreSearch extends Vue {
  ...
  private ok() {
     let createW:any=this.getnew(this.createDate)
      ...
      if(!this.createDate[0]&&this.partCode==""){
        this.$message({message:`日期不可为空，请重新选择`,type:'error'})
        return
      }
      // if(strtW>31){
      //    this.$message({message:'提交日期不可超过一个月，请重新选择',type:'error'})
