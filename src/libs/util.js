...
const showThisMenuEle = (item, access) => {
  if (item.mark && access) {// && access.length
    return access.indexOf(item.mark) > -1
  } else return true
...
export const getNewTagList = (list, newRoute) => {
  ...
  if (newList.findIndex(item => item.name === name) >= 0) {
    newList.forEach((v,i) => {
      if(v.name === name){
        if(JSON.stringify(v.query)===JSON.stringify(query)){
          return newList
        }else{
          v.query = query
          //newList.splice(i,1)
...
/**
 * @param {*} access 用户权限数组，如 ['super_admin', 'business']
 * @param {*} route 路由列表
 */
...
export const getNextName = (list, name) => {
  ...
  if (list.length === 2) {
    ...
    if (list.findIndex(item => item.name === name) === list.length - 1) {
      resObj.res = list[list.length - 2].name
      resObj.query = list[list.length - 2].query
    }
    else{
      resObj.res = list[list.findIndex(item => item.name === name) + 1].name
      resObj.query = list[list.findIndex(item => item.name === name) + 1].query
    }
  }
  return resObj
}
...
// 读取文件，避免重复添加
export function fileReader(files, arr) {
  [].map.call(files, file => {
    let bool = arr.find(child => child.file.name === file.name)
...
// 读取图片，避免重复添加
export function imageReader(files, arr) {
  [].map.call(files, file => {
    let bool = arr.find(child => child.file.name === file.name)
    if (bool) return false
    let reader = new FileReader()
