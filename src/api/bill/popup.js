...
//角色是否为“会计、核算会计、核算主管、核算经理、财务总监”
export function checkMultiple() {
  return axios.request({
    url: `${api.omsSettle}/statement/master/checkMultiple`,
    method: 'get',
  })
}
...
//发票单位 多个
export function ditInvoices(data) {
  return axios.request({
    url: `${api.wmsApi}/guest/findGuestTaxpayerByGuestIds`,
    method: 'post',
    data
  })
}
//开票单位 issuingOffice/findCurrentIssuing
export function findCurrentIssuing(){
  return axios.request({
    url:`${api.omsSettle}/issuingOffice/findCurrentIssuing`,
    method:"get"
  })
}
//不含税对账单号
