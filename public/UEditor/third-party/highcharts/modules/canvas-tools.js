...
*/
function RGBColor (m) {
  this.ok = !1; m.charAt(0) == '#' && (m = m.substr(1, 6)); var m = m.replace(/ /g, ''), m = m.toLowerCase(), a = {aliceblue: 'f0f8ff',
      antiquewhite: 'faebd7',
      aqua: '00ffff',
      aquamarine: '7fffd4',
      azure: 'f0ffff',
      ...
      whitesmoke: 'f5f5f5',
      yellow: 'ffff00',
      yellowgreen: '9acd32'}, c; for (c in a)m == c && (m = a[c]); var d = [{re: /^rgb\((\d{1,3}),\s*(\d{1,3}),\s*(\d{1,3})\)$/, example: ['rgb(123, 234, 45)', 'rgb(255,234,245)'], process: function (b) { return [parseInt(b[1]), parseInt(b[2]), parseInt(b[3])] }}, {re: /^(\w{2})(\w{2})(\w{2})$/,
...
0
        ...
        EM: function (b) { var d = 12, c = new a.Property('fontSize', a.Font.Parse(a.ctx.font).fontSize); c.hasValue() && (d = c.Length.toPixels(b)); return d },
        toPixels: function (d) {
...
a); this.addBezierCurve(b, a, d, d + 1 / 3 * (k - b), c, c + 1 / 3 * (l - a), k, l)
        ...
        var q = [b, a], p = [d, c], t = [k, l], m = [o, n]; this.addPoint(q[0], q[1]); this.addPoint(m[0], m[1]); for (i = 0; i <= 1; i++) {
...
(function (m) {
  var a = m.css, c = m.CanVGRenderer, d = m.SVGRenderer, b = m.extend, k = m.merge, e = m.addEvent, f = m.createElement, g = m.discardElement; b(c.prototype, d.prototype); b(c.prototype, {create: function (a, b, c, d) { this.setContainer(b, c, d); this.configure(a) },
